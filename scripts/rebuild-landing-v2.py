...
path = Path('/home/ubuntu/saude-integrativa-ia-dev/client/src/pages/Home.tsx')
...
source = replace_between(source, 'return (\n    <div className="min-h-screen overflow-hidden pb-32 text-foreground">', '\n}\n\nconst v4HeroDensityContract', shell_return + '\nconst v4HeroDensityContract')

hero = '''function HeroCockpit({ consentComplete, onStartTriage, onOpenDoctors }: { consentComplete: boolean; onStartTriage: () => void; onOpenDoctors: () => void }) {
  return (
...
source = replace_between(source, 'function HeroCockpit({ consentComplete, onStartTriage, onOpenDoctors }', 'function Metric(', hero + 'function Metric(')

home_panel = '''function HomePanel({ onStartTriage, onOpenDoctors }: { onStartTriage: () => void; onOpenDoctors: () => void }) {
  return (
...
source = replace_between(source, 'function HomePanel({ onStartTriage, onOpenDoctors }', 'function TriagePanel(', home_panel + 'function TriagePanel(')
